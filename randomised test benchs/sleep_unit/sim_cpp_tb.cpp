...
int main(int argc, char **argv) {
    ...
    while (sim_time < timeout) {
        ...
        if (sim_time % 10 == 0) {
            ...
            dut->rst_n = rand() % 2;  // Random value between 0 and 1
            dut->if_busy_i = rand() % 2;        // Random value between 0 and 1
            dut->ctrl_busy_i = rand() % 2;      // Random value between 0 and 1
            dut->lsu_busy_i = rand() % 2;       // Random value between 0 and 1
            dut->apu_busy_i = rand() % 2;       // Random value between 0 and 1
            dut->pulp_clock_en_i = rand() % 2;  // Random value between 0 and 1
            dut->p_elw_start_i = rand() % 2;    // Random value between 0 and 1
            dut->p_elw_finish_i = rand() % 2;   // Random value between 0 and 1
            dut->debug_p_elw_no_sleep_i = rand() % 2; // Random value between 0 and 1
            dut->wake_from_sleep_i = rand() % 2; // Random value between 0 and 1
        }
